...
: m__device(torch::kCPU), m__modelPath(), m__vertPath(), m__faceIndices(), m__shapeBlendBasis(), m__poseBlendBasis(),
  m__templateRestShape(), m__jointRegressor(), m__kinematicTree(), m__weights(), m__model(), m__blender(),
  m__regressor(), m__transformer(), m__skinner()
{
}
 ...
 *
 */
SMPL::SMPL(std::string & modelPath, std::string & vertPath, torch::Device & device) noexcept(false)
: m__device(torch::kCPU), m__model(), m__blender(), m__regressor(), m__transformer(), m__skinner()
{
  if(device.has_index())
  {
    m__device = device;
 ...
 *
 */
SMPL::SMPL(const SMPL & smpl) noexcept(false) : m__device(torch::kCPU)
{
  try
  {
    *this = smpl;
...
SMPL & SMPL::operator=(const SMPL & smpl) noexcept(false)
{
  ...
  // soft copy
  //
  if(smpl.m__faceIndices.sizes() == torch::IntArrayRef({FACE_INDEX_NUM, 3}))
  {
    m__faceIndices = smpl.m__faceIndices.clone().to(m__device);
  }

  if(smpl.m__shapeBlendBasis.sizes() == torch::IntArrayRef({VERTEX_NUM, 3, SHAPE_BASIS_DIM}))
  {
    m__shapeBlendBasis = smpl.m__shapeBlendBasis.clone().to(m__device);
  }

  if(smpl.m__poseBlendBasis.sizes() == torch::IntArrayRef({VERTEX_NUM, 3, POSE_BASIS_DIM}))
  {
    m__poseBlendBasis = smpl.m__poseBlendBasis.clone().to(m__device);
  }

  if(smpl.m__jointRegressor.sizes() == torch::IntArrayRef({JOINT_NUM, VERTEX_NUM}))
  {
    m__jointRegressor = smpl.m__jointRegressor.clone().to(m__device);
  }

  if(smpl.m__templateRestShape.sizes() == torch::IntArrayRef({VERTEX_NUM, 3}))
  {
    m__templateRestShape = smpl.m__templateRestShape.clone().to(m__device);
  }

  if(smpl.m__kinematicTree.sizes() == torch::IntArrayRef({2, JOINT_NUM}))
  {
    m__kinematicTree = smpl.m__kinematicTree.clone().to(m__device);
  }

  if(smpl.m__weights.sizes() == torch::IntArrayRef({VERTEX_NUM, JOINT_NUM}))
  {
...
torch::Tensor SMPL::getFaceIndex() noexcept(false)
{
  ...
  if(m__faceIndices.sizes() == torch::IntArrayRef({FACE_INDEX_NUM, 3}))
  {
...
torch::Tensor SMPL::getVertexRaw(int64_t idx) noexcept(false)
{
...
void SMPL::init() noexcept(false)
{
  ...
  if(std::experimental::filesystem::exists(path))
  {
    ...
    m__faceIndices = torch::from_blob(faceIndices.data(), {FACE_INDEX_NUM, 3}, torch::kInt32).clone().to(m__device);
    ...
    m__templateRestShape = torch::from_blob(templateRestShape.data(), {VERTEX_NUM, 3}).to(m__device); // (6890, 3)
    m__jointRegressor = torch::from_blob(jointRegressor.data(), {JOINT_NUM, VERTEX_NUM}).to(m__device); // (24, 6890)

    // transformer
    xt::xarray<int64_t> kinematicTree;
    xt::from_json(m__model["kinematic_tree"], kinematicTree);
    m__kinematicTree = torch::from_blob(kinematicTree.data(), {2, JOINT_NUM}, torch::kInt64).to(m__device); // (2, 24)

    // skinner
    xt::xarray<float> weights;
    xt::from_json(m__model["weights"], weights);
    m__weights = torch::from_blob(weights.data(), {VERTEX_NUM, JOINT_NUM}).to(m__device); // (6890, 24)
  }
  else
  {
...
void SMPL::out(int64_t index) noexcept(false)
{
  ...
  if(vertices.sizes() == torch::IntArrayRef({BATCH_SIZE, VERTEX_NUM, 3})
