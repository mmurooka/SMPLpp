...
: m__device(torch::kCPU), m__restShape(), m__shapeBlendShape(), m__poseBlendShape(), m__templateRestShape(),
  m__joints(), m__jointRegressor()
{
}
...
: m__device(torch::kCPU), m__restShape(), m__shapeBlendShape(), m__poseBlendShape(), m__joints()
{
  if(device.has_index())
  {
    m__device = device;
  ...
  if(jointRegressor.sizes() == torch::IntArrayRef({JOINT_NUM, VERTEX_NUM}))
  {
    m__jointRegressor = jointRegressor.clone().to(m__device);
  }
  else
  {
    throw smpl_error("JointRegression", "Failed to initialize joint regressor!");
  }

  if(templateRestShape.sizes() == torch::IntArrayRef({VERTEX_NUM, 3}))
  {
    m__templateRestShape = templateRestShape.clone().to(m__device);
  }
  else
  {
    throw smpl_error("JointRegression", "Failed to initialize template shape!");
  }
}
 ...
 *
 */
JointRegression::JointRegression(const JointRegression & jointRegression) noexcept(false)
: m__device(torch::kCPU), m__restShape(), m__joints()
{
  try
  {
 ...
 *
 */
JointRegression & JointRegression::operator=(const JointRegression & jointRegression) noexcept(false)
{
  //
  // hard copy
  //
  ...
  if(jointRegression.m__shapeBlendShape.sizes() == torch::IntArrayRef({BATCH_SIZE, VERTEX_NUM, 3}))
  {
    m__shapeBlendShape = jointRegression.m__shapeBlendShape.clone().to(m__device);
  }
  else
  {
  ...
  if(jointRegression.m__poseBlendShape.sizes() == torch::IntArrayRef({BATCH_SIZE, VERTEX_NUM, 3}))
  {
    m__poseBlendShape = jointRegression.m__poseBlendShape.clone().to(m__device);
  }
  else
  {
  ...
  if(jointRegression.m__templateRestShape.sizes() == torch::IntArrayRef({VERTEX_NUM, 3}))
  {
    m__templateRestShape = jointRegression.m__templateRestShape.clone().to(m__device);
  }
  else
  {
  ...
  if(jointRegression.m__jointRegressor.sizes() == torch::IntArrayRef({JOINT_NUM, VERTEX_NUM}))
  {
    m__jointRegressor = jointRegression.m__jointRegressor.clone().to(m__device);
  }
  else
  {
  ...
  // soft copy
  //
  if(jointRegression.m__restShape.sizes() == torch::IntArrayRef({BATCH_SIZE, VERTEX_NUM, 3}))
  {
    m__restShape = jointRegression.m__restShape.clone().to(m__device);
  }

  if(jointRegression.m__joints.sizes() == torch::IntArrayRef({BATCH_SIZE, JOINT_NUM, 3}))
  {
 ...
 *
 */
void JointRegression::setShapeBlendShape(const torch::Tensor & shapeBlendShape) noexcept(false)
{
  if(shapeBlendShape.sizes() == torch::IntArrayRef({BATCH_SIZE, VERTEX_NUM, 3}))
  {
 ...
 *
 */
void JointRegression::setPoseBlendShape(const torch::Tensor & poseBlendShape) noexcept(false)
{
  if(poseBlendShape.sizes() == torch::IntArrayRef({BATCH_SIZE, VERTEX_NUM, 3}))
  {
 ...
 *
 */
void JointRegression::setTemplateRestShape(const torch::Tensor & templateRestShape) noexcept(false)
{
  if(templateRestShape.sizes() == torch::IntArrayRef({VERTEX_NUM, 3}))
  {
    m__templateRestShape = templateRestShape.clone().to(m__device);
 ...
 *
 */
void JointRegression::setJointRegressor(const torch::Tensor & jointRegressor) noexcept(false)
{
  if(jointRegressor.sizes() == torch::IntArrayRef({JOINT_NUM, VERTEX_NUM}))
  {
    m__jointRegressor = jointRegressor.clone().to(m__device);
...
torch::Tensor JointRegression::getRestShape() noexcept(false)
{
  ...
  if(m__restShape.sizes() == torch::IntArrayRef({BATCH_SIZE, VERTEX_NUM, 3}))
  {
...
torch::Tensor JointRegression::getJoint() noexcept(false)
{
  ...
  if(m__joints.sizes() == torch::IntArrayRef({BATCH_SIZE, JOINT_NUM, 3}))
  {
...
void JointRegression::linearCombine() noexcept(false)
{
  if(m__shapeBlendShape.sizes() == torch::IntArrayRef({BATCH_SIZE, VERTEX_NUM, 3})
     && m__poseBlendShape.sizes() == torch::IntArrayRef({BATCH_SIZE, VERTEX_NUM, 3})
     && m__templateRestShape.sizes() == torch::IntArrayRef({VERTEX_NUM, 3}))
  {
    m__restShape = m__templateRestShape + m__shapeBlendShape + m__poseBlendShape;
  }
  else
  {
...
void JointRegression::jointRegress() noexcept(false)
{
  if(m__shapeBlendShape.sizes() == torch::IntArrayRef({BATCH_SIZE, VERTEX_NUM, 3})
     && m__templateRestShape.sizes() == torch::IntArrayRef({VERTEX_NUM, 3}))
  {
    torch::Tensor blendShape = m__templateRestShape + m__shapeBlendShape; // (N, 6890, 3)
    m__joints = torch::tensordot(blendShape, m__jointRegressor, {1}, {1}); // (N, 3, 24)
    m__joints = torch::transpose(m__joints, 1, 2); // (N, 24, 3)
  }
  else
  {
    throw smpl_error("JointRegression", "Cannot regress vertices to joints!");
  }
