  ...
  m__poseRot(), m__restPoseRot(), m__poseBlendBasis(), m__poseBlendShape()
{
  ...
  if(shapeBlendBasis.sizes() == torch::IntArrayRef({VERTEX_NUM, 3, SHAPE_BASIS_DIM}))
  {
    m__shapeBlendBasis = shapeBlendBasis.clone().to(m__device);
  }
  else
  {
    throw smpl_error("BlendShape", "Failed to initialize shape blend basis!");
  }

  if(poseBlendBasis.sizes() == torch::IntArrayRef({VERTEX_NUM, 3, POSE_BASIS_DIM}))
  {
    m__poseBlendBasis = poseBlendBasis.clone().to(m__device);
  }
  else
  {
    throw smpl_error("BlendShape", "Failed to initialize pose blend basis!");
  }
}
 ...
 *
 */
BlendShape::BlendShape(const BlendShape & blendShape) noexcept(false) : m__device(torch::kCPU)
{
  try
  {
    *this = blendShape;
...
BlendShape & BlendShape::operator=(const BlendShape & blendShape) noexcept(false)
{
  ...
  if(blendShape.m__shapeBlendBasis.sizes() == torch::IntArrayRef({VERTEX_NUM, 3, SHAPE_BASIS_DIM}))
  {
    m__shapeBlendBasis = blendShape.m__shapeBlendBasis.clone().to(m__device);
  }
  else
  {
  ...
  if(blendShape.m__poseBlendBasis.sizes() == torch::IntArrayRef({VERTEX_NUM, 3, POSE_BASIS_DIM}))
  {
  ...
  if(blendShape.m__beta.sizes() == torch::IntArrayRef({BATCH_SIZE, SHAPE_BASIS_DIM}))
  {
  ...
  if(blendShape.m__restTheta.sizes() == torch::IntArrayRef({BATCH_SIZE, JOINT_NUM, 3}))
  {
  ...
  //  soft copy
  //
  if(blendShape.m__shapeBlendShape.sizes() == torch::IntArrayRef({BATCH_SIZE, VERTEX_NUM, 3}))
  {
    m__shapeBlendShape = blendShape.m__shapeBlendShape.clone().to(m__device);
  }

  if(blendShape.m__poseBlendShape.sizes() == torch::IntArrayRef({BATCH_SIZE, VERTEX_NUM, 3}))
  {
    m__poseBlendShape = blendShape.m__poseBlendShape.clone().to(m__device);
  }

  if(blendShape.m__poseRot.sizes() == torch::IntArrayRef({BATCH_SIZE, JOINT_NUM, 3, 3}))
  {
    m__poseRot = blendShape.m__poseRot.clone().to(m__device);
  }

  if(blendShape.m__restPoseRot.sizes() == torch::IntArrayRef({BATCH_SIZE, JOINT_NUM, 3, 3}))
  {
 ...
 *
 */
void BlendShape::setShapeBlendBasis(const torch::Tensor & shapeBlendBasis) noexcept(false)
{
  if(shapeBlendBasis.sizes() == torch::IntArrayRef({VERTEX_NUM, 3, SHAPE_BASIS_DIM}))
  {
    m__shapeBlendBasis = shapeBlendBasis.clone().to(m__device);
...
{
  if(theta.sizes() == torch::IntArrayRef({BATCH_SIZE, JOINT_NUM, 3}))
  {
 ...
 *
 */
void BlendShape::setRestTheta(const torch::Tensor & restTheta) noexcept(true)
{
  if(restTheta.sizes() == torch::IntArrayRef({BATCH_SIZE, JOINT_NUM, 3}))
  {
 ...
 *
 */
void BlendShape::setPoseBlendBasis(const torch::Tensor & poseBlendBasis) noexcept(false)
{
  if(poseBlendBasis.sizes() == torch::IntArrayRef({VERTEX_NUM, 3, POSE_BASIS_DIM}))
  {
    m__poseBlendBasis = poseBlendBasis.clone().to(m__device);
...
torch::Tensor BlendShape::getShapeBlendShape() noexcept(false)
{
  ...
  if(m__shapeBlendShape.sizes() == torch::IntArrayRef({BATCH_SIZE, VERTEX_NUM, 3}))
  {
...
torch::Tensor BlendShape::getPoseRotation() noexcept(false)
{
  ...
  if(m__poseRot.sizes() == torch::IntArrayRef({BATCH_SIZE, JOINT_NUM, 3, 3}))
  {
...
torch::Tensor BlendShape::getRestPoseRotation() noexcept(false)
{
  ...
  if(m__restPoseRot.sizes() == torch::IntArrayRef({BATCH_SIZE, JOINT_NUM, 3, 3}))
  {
...
torch::Tensor BlendShape::getPoseBlendShape() noexcept(false)
{
  ...
  if(m__poseBlendShape.sizes() == torch::IntArrayRef({BATCH_SIZE, VERTEX_NUM, 3}))
  {
...
{
  if(m__shapeBlendBasis.sizes() == torch::IntArrayRef({VERTEX_NUM, 3, SHAPE_BASIS_DIM})
     && m__beta.sizes() == torch::IntArrayRef({BATCH_SIZE, SHAPE_BASIS_DIM}))
  {
    m__shapeBlendShape = torch::tensordot(m__beta, m__shapeBlendBasis, {1}, {2}); // (N, 6890, 3)
  }
  else
  {
...
void BlendShape::poseBlend() noexcept(false)
{
  ...
  //
  if(m__theta.sizes() == torch::IntArrayRef({BATCH_SIZE, JOINT_NUM, 3}))
  {
    try
    {
  ...
  if(!m__restTheta.is_same(torch::Tensor()) && m__restTheta.sizes() == torch::IntArrayRef({BATCH_SIZE, JOINT_NUM, 3}))
  {
    try
    {
  ...
  else
  {
    ...
    m__restPoseRot = m__restPoseRot.expand({BATCH_SIZE, JOINT_NUM, 3, 3}); // (N, 24, 3, 3)
  }
  ...
  // pose blend
  //
  if(m__poseBlendBasis.sizes() == torch::IntArrayRef({VERTEX_NUM, 3, POSE_BASIS_DIM}))
  {
    m__poseBlendShape = torch::tensordot(poseBlendCoeffs, m__poseBlendBasis, {1}, {2}); // (N, 6890, 3)
  }
  else
  {
    throw smpl_error("BlendShape", "Cannot blend pose-dependented shape!");
 ...
 *
 */
torch::Tensor BlendShape::rodrigues(torch::Tensor & theta) noexcept(false)
{
  if(theta.sizes() != torch::IntArrayRef({BATCH_SIZE, JOINT_NUM, 3}))
  {
  ...
  //
  constexpr double eps = 1e-8; // small value to avoid zero division
 ...
 *
 */
torch::Tensor BlendShape::unroll(torch::Tensor & rotation) noexcept(false)
{
  if(rotation.sizes() != torch::IntArrayRef({BATCH_SIZE, JOINT_NUM, 3, 3}))
  {
    throw smpl_error("BlendShape", "Cannot unroll a rotation!");
  }

  torch::Tensor unRotation = torch::reshape(rotation, {BATCH_SIZE, JOINT_NUM * 3 * 3});
