...
private: // PRIVATE METHODS
  // %% Transformations %%
  torch::Tensor localTransform(torch::Tensor & poseRotHomo) noexcept(false);
  torch::Tensor globalTransform(torch::Tensor & localTransformations) noexcept(false);
  void relativeTransform(torch::Tensor & globalTransformations) noexcept(false);

protected: // PROTECTED METHODS
public: // PUBLIC METHODS
  // %% Constructor and Destructor %%
  WorldTransformation() noexcept(true);
  WorldTransformation(torch::Tensor & kineTree, torch::Device & device) noexcept(false);
  WorldTransformation(const WorldTransformation & worldTransformation) noexcept(false);
  ~WorldTransformation() noexcept(true);

  // %% Operators %%
  WorldTransformation & operator=(const WorldTransformation & worldTransformation) noexcept(false);
