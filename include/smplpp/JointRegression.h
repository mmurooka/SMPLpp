...
public: // PUBLIC METHODS
  ...
  // %% Operators %%
  JointRegression & operator=(const JointRegression & jointRegression) noexcept(false);

  // %% Getter and Setter %%
  void setDevice(const torch::Device & device) noexcept(false);
  void setShapeBlendShape(const torch::Tensor & shapeBlendShape) noexcept(false);
  void setPoseBlendShape(const torch::Tensor & poseBlendShape) noexcept(false);
  void setTemplateRestShape(const torch::Tensor & templateRestShape) noexcept(false);
  void setJointRegressor(const torch::Tensor & jointRegressor) noexcept(false);
